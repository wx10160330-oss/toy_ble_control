...
> 💡 **懒人模式**：插件自带一个交互式协议探测器，直接访问 `http://服务器IP:5122/probe` 就能用——它会帮你做下面这些事的 90%：自动连玩具、列出 Write/Notify 特征（OTA 通道自动标红警告）、引导你录制每个档位的 Notify 字节、自动对比所有样本找出 mode/intensity 位置、最后直接生成一段可粘贴到配置面板的 JSON。手抄字节、人眼对比的活基本省了，只剩"用官方 APP 把玩具调到对应档位然后回来点捕获"这一步还需要你动手。**强烈建议先试试探测器**，看完不顺手再回来按下文一步步手动操作。
