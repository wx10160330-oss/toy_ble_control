...
### 1. 服务器
能运行 AstrBot 的 Python 3.8+ 环境。

### 2. 「中继客户端」设备 —— 手机 / 平板 / 电脑都行
中继客户端就是真正去用蓝牙连玩具的那台机器。**它必须放在玩具身边（蓝牙有效距离 ~10 米）**，云服务器再远也没关系。

只要是支持 Web Bluetooth API 的浏览器都能用：

| 设备 + 浏览器 | 支持情况 |
|---|---|
| Android + Chrome / Edge | ✅ 原生支持 |
| **Windows + Chrome / Edge** | ✅ 原生支持 |
| **Mac + Chrome / Edge** | ✅ 原生支持（Mac 自带蓝牙） |
| Linux + Chrome（带蓝牙的电脑 / 加 USB 蓝牙适配器） | ✅ 原生支持 |
| Mac + Safari | ❌ Safari 不支持 Web Bluetooth |
| iOS + Safari | ❌ Safari 不支持 |
| iOS + [Bluefy](https://apps.apple.com/app/bluefy-web-ble-browser/id1492822055) | ⚠️ 第三方付费浏览器，能用但体验一般 |
| 任意系统 + Firefox | ❌ Firefox 不支持 |

> **iPhone 用户怎么办**：
> - **推荐**：随便找台带蓝牙的电脑（Mac / Windows / Linux 都行，没蓝牙的台式机插一个十几块的 USB 蓝牙适配器即可），用 Chrome 或 Edge 打开中继页就行，用法和手机完全一致。
> - **凑合**：iPhone 装 Bluefy 浏览器（App Store 搜得到），但是要付费且体验不如 Chrome。

### 3. HTTPS 要求（手机 / 电脑 Chrome 用户都看）

Chrome / Edge 的 Web Bluetooth API **只允许在以下两种地址下运行**：
- ✅ `https://...`（带证书）
- ✅ `http://localhost` 或 `http://127.0.0.1`（同机）
- ❌ `http://<IP>:5122` 或 `http://<公网域名>:5122`（**纯 HTTP + 非 localhost 会被拒绝，点连接按钮没反应**）

如果你的中继页是部署在云服务器上、客户端通过 `http://公网IP:5122` 访问，**电脑端、手机端 Chrome 都会拒绝**（实测手机端也一样要走下面的解法）。

**两种解法**：

**解法 A（临时、适合朋友临时玩一次，手机 / 电脑都行）**
把你的服务器 URL 加进浏览器白名单。Chrome / Edge 都支持，步骤一模一样，只是地址栏前缀不同：
...
1. 上面表里对应的地址输入浏览器地址栏，回车
2. 把 `http://<你的服务器IP>:5122` 填到 **"Insecure origins treated as secure"** 输入框
3. 下方下拉框选 `Enabled`，**完全退出并重启浏览器**（手机上要从最近任务里划掉再重开）
4. 重新访问中继页，点【连接玩具】应该能弹出蓝牙选择框
5. **缺点**：每台设备都得单独配；安全性下降（这个站对该浏览器来说会被当成 HTTPS 看待）

**解法 B（一劳永逸、推荐）**
给你的云服务器配域名 + Let's Encrypt 免费证书，把 HTTP 改成 HTTPS：
1. 买（或用免费的）域名指向你的云服务器 IP
2. 服务器装 nginx + certbot：`certbot --nginx -d your.domain.com`
3. nginx 反代 `https://your.domain.com` → `http://127.0.0.1:5122`
4. 所有人（手机 / 电脑 / 全平台）访问 `https://your.domain.com` 即可，无需任何配置

### 4. Android Chrome 用户的「隐藏前置权限三件套」

Android 上 Chrome 用 Web Bluetooth 扫描设备，系统层面**同时需要这三个开关**——少一个就**直接显示空设备列表、不报错、不提示**，非常坑爹：

- ☑️ **蓝牙开关** — 系统设置 / 下拉栏开
- ☑️ **位置开关（GPS）** — 系统设置 / 下拉栏的「位置信息」总开关打开
- ☑️ **Chrome 的位置权限** — 设置 → 应用 → Chrome → 权限 → 位置 → 允许（最好选「仅在使用时允许」就够了）

> **为啥扫蓝牙要给位置权限？** Google 把 BLE 扫描视为「可推断用户位置」的隐私敏感操作（蓝牙信标可以反查地点），所以 Android 6.0 起强制要求扫描方持有位置权限，不给就静默失败。这是系统级行为，不是本插件的设计。

**症状自检**：如果你在中继页点【连接玩具】、弹出的蓝牙选择框**一直是空的**（没有任何设备名出现，包括路由器、耳机这种常见 BLE 设备），90% 是这三件套缺了。把三个开关检查一遍重试。

### 5. 网络
中继客户端要能访问到服务器的 HTTP 端口。本地局域网直接用 `http://服务器IP:5122`；外网需要内网穿透 / 反向代理（cpolar、frp、natapp、Cloudflare Tunnel 都行，**Cloudflare Tunnel 还顺便给你 HTTPS**，对电脑端 Chrome 用户最友好）。

### 6. 蓝牙设备
任意支持 BLE GATT 写入控制的玩具。
...
| `access_password` | string | **必填**（v2.7.0 起）。中继网页的访问密码。留空则 HTTP 服务不启动。详见下方「访问密码」一节 |
...
### 访问密码（`access_password`，v2.7.0 起必填）

考虑到中继网页可以直接下发控制指令、还能看到设备状态，v2.7.0 起加了一层轻量访问控制：

- **留空 = 插件启动后不开 HTTP 服务**，中继网页整个进不去；日志里会有一条错误提醒你去补密码。
- **填了密码** = 打开中继页会先跳到登录页，输对密码才能进入操作界面。登录状态用 httpOnly Cookie 记录（有效期 7 天），插件重启会全部失效。
- **防暴力**：同一 IP 连续输错 5 次密码会被锁 60 秒。

**建议**：至少 8 位，字母 + 数字混合。密码本身只存在插件配置里，不写日志。

> 注意：如果你用的是 HTTP 明文（默认场景），密码会以明文经过局域网。同一 Wi-Fi 下小心信任的场景下够用；如果要外网访问，请配合 Cloudflare Tunnel / frp + HTTPS 使用（详见后面「HTTPS 要求」一节）。
...
| **强度最小值 / 最大值** | 同上 | 1 / 3 |
| **模式描述** *(v2.6.0 新增，可选)* | 多行文本，用自然语言写清楚每个模式对应的感觉 / 节奏。留空则不显示，不影响功能。<br/>**用途**：<br/>① 会随 `toy_ble_list_functions` 工具返回给 LLM，AI 就能"看懂"你偏好哪个模式，选起来更准；<br/>② 中继网页的功能卡片上会显示一个「模式说明」小卡片，方便你自己对照挑档位。 | `模式1：持续稳定震动`<br/>`模式3：波浪形，先慢后快再慢，像海浪起伏`<br/>`模式5：快一下慢一下，快速震动后停顿再快速震动`<br/>`模式7：渐强渐弱循环` |
...
任意一台**放在玩具旁边、带蓝牙、装了 Chrome / Edge 的设备**都行（手机 / 平板 / 笔记本 / 桌面机均可，详见[三、环境要求](#三环境要求)）。浏览器访问 `http://<服务器IP>:<port>`（如 `http://192.168.1.100:5122`）：

1. 点击 **「连接设备」**。
2. 弹窗里选你的玩具。
3. 状态变绿即连接成功。
4. **保持此网页在前台、不要锁屏**（锁屏 / 切后台会暂停 JS 轮询）。

> **没弹设备选择框 / 点了连接按钮没反应**？看[三、3 HTTPS 要求](#3-https-要求手机--电脑-chrome-用户都看)那一节。
>
> **弹出来了但设备列表是空的**（Android）？看[三、4 Android 权限三件套](#4-android-chrome-用户的隐藏前置权限三件套)。
...
| 现象 | 排查方向 |
|------|----------|
| 浏览器搜不到设备 / 设备列表是空的 | 综合排查：①玩具已开机、没被别的 APP 占用；②浏览器支持 Web Bluetooth（iOS Safari 不行，详见[三、2](#2-中继客户端设备--手机--平板--电脑都行)）；③HTTPS / `chrome://flags` 已配（详见[三、3](#3-https-要求手机--电脑-chrome-用户都看)）；④Android 用户检查「权限三件套」=蓝牙开+GPS 开+Chrome 位置权限（详见[三、4](#4-android-chrome-用户的隐藏前置权限三件套)） |
