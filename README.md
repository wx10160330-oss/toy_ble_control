...
> 💡 **懒人模式**：插件自带一个交互式协议探测器，直接访问 `http://服务器IP:5122/probe` 就能用——它会帮你做最痛苦的两件事：**自动对比所有样本的字节差异 + 自动生成可粘贴到配置面板的 JSON**。你可以两种方式喂给它字节数据：（A）按下文用 nRF Connect 手动捕获，然后把抓到的字节粘到表格里——**最稳定，强烈推荐**；（B）让探测器自己连玩具、按"捕获 Notify"自动读——快但不稳定，取决于玩具固件实现。两种方式可以混用。**强烈建议先打开探测器**看看，再回来按下文逐步操作。
