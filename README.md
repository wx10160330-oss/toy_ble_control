...
如果你的中继页是部署在云服务器上、客户端通过 `http://公网IP:5122` 访问，电脑端 Chrome 大概率连不上玩具。**手机端 Chrome 比电脑端更宽松**，有时纯 HTTP 也能用，但电脑端基本不行。

**两种解法**：

**解法 A（临时、适合朋友临时玩一次）**
把你的服务器 URL 加进 Chrome 白名单：
1. 浏览器地址栏访问 `chrome://flags/#unsafely-treat-insecure-origin-as-secure`
2. 把 `http://<你的服务器IP>:5122` 填到 "Insecure origins treated as secure" 输入框
3. 下方下拉框选 `Enabled`，重启浏览器
4. **缺点**：每台电脑都得单独配，朋友嫌麻烦；安全性下降（这个站对该用户来说会被当成 HTTPS 看待）
...
> **没弹设备选择框 / 点了连接按钮没反应**？大概率是 HTTPS 问题，看[三、3 HTTPS 要求](#3-https-要求电脑端-chrome-用户必看)那一节。
