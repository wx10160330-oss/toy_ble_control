...
> 默认值 `false` 跟 v1.0.0 老版本完全一致，老 Svakom 用户**不用改这个开关**。
...
5. **重要**：看这个特征的 Properties。如果只有 `WRITE-WITHOUT-RESPONSE` 而没有 `WRITE`，把 `write_without_response` 设为 `true`；只有 `WRITE` 就保持默认 `false`。**写错了会出现「设备已连接但毫无反应」**。详见上文「写入方式」一节。
